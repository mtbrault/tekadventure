...
** Last update Sat May 13 19:25:22 2017 LAABID Zakaria
*/
...
t_level		**gen_event_space(t_level **level, char **conf, int index)
{
  ...
  if ((level[index]->event = malloc(sizeof(t_event) * size_event)) == NULL)
    return (NULL);
  while (i < size_event)
...
t_level		**gen_telep_space(t_level **level, char **conf, int index)
{
  ...
  if ((level[index]->tp = malloc(sizeof(t_tp) * size_tp)) == NULL)
    return (NULL);
  while (i < size_tp)
...
t_level		**gen_mob_space(t_level **level, char **conf, int index)
{
  ...
  if ((level[index]->mob = malloc(sizeof(t_mob) * size_mob)) == NULL)
    return (NULL);
  while (i < size_mob)
    {
...
t_level		**gen_config_space(t_level **level, char **conf)
{
  ...
  i = 0;
  if ((level = malloc(sizeof(t_level) * size_level)) == NULL)
    return (NULL);
  while (i < size_level)
