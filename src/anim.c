...
** Last update Wed May  3 20:08:31 2017 Matthieu BRAULT
*/
...
int		move_perso(sfVector2f vector, t_player *p, sfRenderWindow *window)
{
  ...
  if (vector.x == 0 && vector.y == 0)
    {
      sprite = get_static_char(p->classe_texture, ((sfVector2i)
	{p->dir, p->class}), p->pos2, ((sfVector2i) {8, 4}));
      sfRenderWindow_drawSprite(window, sprite, NULL);
      return ;
    }
  else if (i.x == 0)
  ...
  move = ((sfVector2f) {move.x + axe.x, move.y + axe.y});
  sprite = get_static_char(p->spriteboard, ((sfVector2i)
    {loop.y, p->dir}), p->pos2, ((sfVector2i) {6, 8}));
  sfRenderWindow_drawSprite(window, sprite, NULL);
  loop.x = loop.x + 1;
  if (loop.y % 100 == 0)
    loop.y = loop.y + 1;
  p->pos2 = ((sfVector2f) {p->pos2.x + move.x, p->pos2.y + move.y});
  if (i.y > i.x)
    {
      i = ((sfVector2i) {0, 0});
      loop = ((sfVector2i) {0, 0});
