...
** Last update Fri May 26 22:50:17 2017 Capitaine CASSE
*/
  ...
  while (x <= getconf_index(conf, MOBS_NB, y))
    {
      ...
      while (conf[i + 2] != NULL)
	{
	  if (my_strncmp(MOBS_NAME, conf[i + 1], L_NAME) == 0)
	    level[y]->mob[x - 1]->name = unquote(conf[i + 1] + L_NAME + 1);
	  if (my_strncmp(MOBS_TEXT, conf[i + 2], L_TEXT) == 0)
	    level[y]->mob[x - 1]->texture = unquote(conf[i + 2] + L_TEXT + 1);
  ...
  while (x <= getconf_index(conf, MOBS_NB, y))
    {
      ...
      while (conf[i] != NULL)
	{
	  if (my_strncmp(MOBS_LIFE, conf[i + 3], L_LIFE) == 0)
	    level[y]->mob[x - 1]->life = nbrepur((conf[i + 3] + L_LIFE));
	  if (my_strncmp(MOBS_DMG, conf[i + 4], L_DMG) == 0)
	    level[y]->mob[x - 1]->damage = nbrepur((conf[i + 4] + L_DMG));
	  if ((my_strncmp(end, conf[i], my_strlen(end))) == 0)
	    break;
	  else if ((my_strncmp(EVENT, conf[i], L_EVENT)) == 0 ||
		   ((my_strncmp(CONF_LEVEL, conf[i], L_CONF)) == 0) ||
